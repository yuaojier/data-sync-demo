...
public abstract class AbstractInterceptor implements MethodInterceptor {
    ...
    public boolean isCglibAopProxy(MethodInvocation invocation) {
        if (invocation instanceof ProxyMethodInvocation)
            return AopUtils.isCglibProxy(((ProxyMethodInvocation) invocation).getProxy());
        return AopUtils.isCglibProxy(invocation.getThis());
    }
    ...
    public <T> T getValueByParameterAnnotation(MethodInvocation invocation, Class<?> parameterAnnotationType, Class<T> parameterType) {
        ...
        String parameterTypesValue = ArrayUtils.toString(parameterTypes);
        Annotation[][] parameterAnnotations = method.getParameterAnnotations();
        Object[] arguments = invocation.getArguments();

        if (ArrayUtils.isEmpty(parameterAnnotations)) {
            throw new AopConfigException("Not found any annotations");
        }
        ...
        for (Annotation[] parameterAnnotation : parameterAnnotations) {
            for (Annotation annotation : parameterAnnotation) {
                if (annotation.annotationType() == parameterAnnotationType) {
                    ...
                    if (value != null) {
                        throw new AopConfigException("Only 1 annotation=" + parameterAnnotationType.getName() + " can be added in method [name=" + method.getName() + ", parameterTypes=" + parameterTypesValue + "]");
                    }

                    Object object = arguments[valueIndex];
                    // 方法注解的值不允许为空
                    if (object == null) {
                        throw new AopConfigException("Value for annotation=" + parameterAnnotationType.getName() + " in method [name=" + method.getName() + ", parameterTypes=" + parameterTypesValue + "] is null");
                    }

                    // 方法注解的类型不匹配
                    if (object.getClass() != parameterType) {
                        throw new AopConfigException("Type for annotation=" + parameterAnnotationType.getName() + " in method [name=" + method.getName() + ", parameterTypes=" + parameterTypesValue + "] must be " + parameterType.getName());
                    }
        ...
        if (annotationIndex == 0) {
            throw new AopConfigException("Not found annotation=" + parameterAnnotationType.getName() + " in method [name=" + method.getName() + ", parameterTypes=" + parameterTypesValue + "]");
        }
